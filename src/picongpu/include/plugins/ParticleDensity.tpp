...
struct ParticleDensityKernel
{
    ...
    DINLINE void operator()(FramePtr frame, uint16_t particleID, Field field, const ::PMacc::math::Int<3>& blockCellIdx)
    {
        ...
        /// \warn reduce a normalized float_X with particleAccess::Weight() / particles::TYPICAL_NUM_PARTICLES_PER_MACROPARTICLE
...
void ParticleDensity<ParticlesType>::notify(uint32_t currentStep)
{
    ...
    ///       to avoid overflows for heavy weightings
    ///
    /// \warn multiply normalized float_X "count" with particles::TYPICAL_NUM_PARTICLES_PER_MACROPARTICLE here (as double!)
    ///
    file << globalDensity;
}
