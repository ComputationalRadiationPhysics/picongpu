...
namespace picongpu
{
  namespace radiation_observer
  {
    DINLINE vec2 observation_direction(const int theta_id_extern)
    {
      ...
      const numtype2 theta(  my_index_theta * delta_angle  + 0.5*picongpu::PI - angle_range); // off axis angle
      const numtype2 phi(    my_index_phi   * delta_angle  - angle_range); // off axis angle

      return vec2( sinf(theta)*cosf(phi) , sinf(theta)*sinf(phi) , cosf(theta) ) ;


      /*
      if(theta_id_extern < (parameters::N_theta/2)) // scan along x-axis
	{
