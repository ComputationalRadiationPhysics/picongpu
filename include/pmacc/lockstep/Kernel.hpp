    ...
    namespace exec
    {
        namespace detail
        {
            ...
            struct LockStepKernel : private T_Kernel
            {
                 ...
                 *            @attention acc can not be a const reference else gcc shows an an compiler error "use of
                 * uninitialized variable" during the CI checks. This error is only shown if we compile in release
                 * mode, in debug all is fine. This is most likely a compiler bug.
                 *            @todo please check if we can use a reference and which gcc versions are problematic.
