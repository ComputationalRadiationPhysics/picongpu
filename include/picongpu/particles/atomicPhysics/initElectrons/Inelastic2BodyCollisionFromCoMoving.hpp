...
namespace picongpu::particles::atomicPhysics::initElectrons
{
    ...
    private:
        ...
        {
            // UNIT_MASS * sim.unit.length() / sim.unit.time(), **weighted**
            ...
            // (UNIT_MASS * sim.unit.length() / sim.unit.time(), weighted) / weighting
            //      / sqrt( (UNIT_MASS^2 * sim.unit.length()^2 / sim.unit.time()^2, not weighted)
            //              + (UNIT_MASS^2 * sim.unit.length()^2 / sim.unit.time()^2, not weighted) )
            ...
            // ((UNIT_MASS^2 * sim.unit.length()^2 / sim.unit.time()^2, not weighted)
            //  / (UNIT_MASS^2 * sim.unit.length()^2 / sim.unit.time()^2, not weighted)) + unitless
            // unitless, not weighted
            float_64 const gamma = math::sqrt(momentumSquared_Lab / m2c2 + 1.0);

            // (UNIT_MASS^2 * sim.unit.length()^2 / sim.unit.time()^2, not weighted)
            //  / ((UNIT_MASS^2 * sim.unit.length()^2 / sim.unit.time()^2, not weighted) * (unitless, not weighted)^2)
            // unitless, not weighted
    ...
    public:
            ...
            T_RngGeneratorFloat& rngGenerator)
        {
            ...
            // UNIT_MASS * sim.unit.length() / sim.unit.time(), not weighted
            ...
            // weight * (unitless^2 - unitless) * (UNIT_MASS * sim.unit.length()/sim.unit.time())
            // = UNIT_MASS * sim.unit.length() / sim.unit.time(), weighted
