...
namespace picongpu::particles::atomicPhysics::initElectrons
{
    ...
    private:
        ...
        HDINLINE static detail::LorentzBoost getLorentzBoostFromIonToLab(T_IonParticle& ion)
        {
            ...
            // UNIT_MASS^2 * UNIT_LENGTH^2 / sim.unit.time()^2, not weighted
            float_64 const m2c2
                = pmacc::math::cPow(mass * picongpu::SI::SPEED_OF_LIGHT_SI / (UNIT_LENGTH / sim.unit.time()), 2u);

            // (UNIT_MASS * UNIT_LENGTH / sim.unit.time(), weighted) / weighting
            //      / sqrt( (UNIT_MASS^2 * UNIT_LENGTH^2 / sim.unit.time()^2, not weighted)
            //              + (UNIT_MASS^2 * UNIT_LENGTH^2 / sim.unit.time()^2, not weighted) )
            ...
            // ((UNIT_MASS^2 * UNIT_LENGTH^2 / sim.unit.time()^2, not weighted)
            //  / (UNIT_MASS^2 * UNIT_LENGTH^2 / sim.unit.time()^2, not weighted)) + unitless
            // unitless, not weighted
            float_64 const gamma = math::sqrt(momentumSquared_Lab / m2c2 + 1.0);

            // (UNIT_MASS^2 * UNIT_LENGTH^2 / sim.unit.time()^2, not weighted)
            //  / ((UNIT_MASS^2 * UNIT_LENGTH^2 / sim.unit.time()^2, not weighted) * (unitless, not weighted)^2)
            // unitless, not weighted
    ...
    public:
            ...
            T_RngGeneratorFloat& rngGenerator)
        {
            ...
            // UNIT_MASS * UNIT_LENGTH / sim.unit.time(), not weighted
            float_64 const mc_Electron
                = massElectron * picongpu::SI::SPEED_OF_LIGHT_SI / (UNIT_LENGTH / sim.unit.time());

            // norm of electron momentum after inelastic collision in Ion-System
            // weight * (unitless^2 - unitless) * (UNIT_MASS * UNIT_LENGTH/sim.unit.time())
            // = UNIT_MASS * UNIT_LENGTH / sim.unit.time(), weighted
            ...
            // UNIT_MASS * UNIT_LENGTH / sim.unit.time(), not weighted
            float_64 const mc_Ion = massIon * picongpu::SI::SPEED_OF_LIGHT_SI / (UNIT_LENGTH / sim.unit.time());
