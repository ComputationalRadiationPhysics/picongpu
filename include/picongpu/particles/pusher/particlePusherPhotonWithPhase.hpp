 ...
 *                     Alexander Grund, Richard Pausch, Pawel Ordyna
 *
...
namespace picongpu
{
    namespace particlePusherPhotonWithPhase
    {
        ...
        struct Push
        {
                ...
                const uint32_t)
            {
                ...
                // TODO: Maybe do some part of it with double preccision?
                weighting::type const weighting = particle[weighting_];
                float_X omega = mom_abs / HBAR / weighting * SPEED_OF_LIGHT;
                float_X dPhase = fmod(omega * DELTA_T, 2.0_X * pmacc::math::Pi<float_X>::value);
                float_X newPhase = particle[phase_] - dPhase;
                // Keep in range of [0,2*PI)
                if(newPhase < 0)
                    newPhase += 2._X * pmacc::math::Pi<float_X>::value;
                particle[phase_] = newPhase;
            }

            static pmacc::traits::StringProperty getStringProperties()
            {
                pmacc::traits::StringProperty propList("name", "other");
                propList["param"] = "free streaming photon pusher with phase update";
                return propList;
            }
        };
    } // namespace particlePusherPhotonWithPhase
