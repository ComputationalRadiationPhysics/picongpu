 ...
 *
 * based on previous implementation by Richard Pausch
 *
...
namespace picongpu::particles::manipulators::unary::acc
{
    ...
    struct SparserMacroParticlesPerSuperCell
    {
            ...
            T_Particle& particle)
        {
            ...
            for(uint8_t i = 0; i < picongpu::simDim; ++i)
            {
                keepParticle = keepParticle
                    && ((particleOffsetInCellsToTotalOrigin[i] % superCellSize_RT[i]) == idxCellToKeep_RT[i]);
            }
        ...
        static constexpr std::string_view name = "SparserMacroParticlesPerSuperCell";
    };
} // namespace picongpu::particles::manipulators::unary::acc
