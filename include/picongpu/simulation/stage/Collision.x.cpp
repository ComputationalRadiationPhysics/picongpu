...
namespace picongpu::simulation::stage
{
    namespace collision
    {
        ...
        struct CallColliders
        {
            ...
            {
                (particles::collision::CallCollider<pmacc::mp_at_c<particles::collision::CollisionPipeline, I>, I>{}(
                     deviceHeap,
                     currentStep),
                 ...);
            }
        }; // namespace collision[]struct CallColliders
    ...
    void Collision::debug(uint32_t const currentStep, const std::shared_ptr<FieldTmp>& screeningLengthSquared)
    {
        ...
        if constexpr(particles::collision::debugScreeningLength)
        {
            ...
            using SqrtBox
                = DataBoxUnaryTransform<typename GridBuffer<float1_X, simDim>::DataBoxType, collision::Sqrt1DimVector>;
            ...
            reduce(pmacc::math::operation::Add(), &reducedCellAmount, &localCells, 1, mpi::reduceMethods::Reduce());
    ...
    void Collision::operator()(uint32_t const currentStep) const
    {
        ...
        if constexpr(numScreeningSpecies > 0u)
        {
            ...
            screeningLengthSquared->modifyByField<CORE + BORDER, collision::GetSquaredScreeningLength>(*maxDensity);
