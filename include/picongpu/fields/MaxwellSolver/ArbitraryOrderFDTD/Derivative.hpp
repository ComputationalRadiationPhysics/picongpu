/* Copyright 2020 Klaus Steiniger, Sergei Bastrakov
 *
...
namespace detail
{
    ...
    struct GeneralAofdtdDerivative
    {
            ...
            T_DataBox const & data ) const
        {
            ...
            lowerIndex[ T_direction ] = static_cast< int32_t >( T_upperNeighbors ) - static_cast< int32_t >( T_neighbors );
            // upperIndex: 1 if ( Forward ) else 0
            upperIndex[ T_direction ] = static_cast< int32_t >( T_neighbors ) - static_cast< int32_t >( T_lowerNeighbors );

            AOFDTDWeights< T_neighbors > const weights{};

            // shortest distance finite difference as initial value
            auto finiteDifference = weights[ 0 ] *
                ( data( upperIndex ) - data( lowerIndex ) );
    ...
    /**@{*/
    /** Functors for forward and backward derivative along the given direction used in ArbitraryOrderFDTD solver
     *
     ...
     *
     * This finite difference approximations for the forward and backward derivative are computed on a staggered grid.
     * That is, the forward derivative will be known at a position i+1/2, if the field f is known
     * at 2 * T_neighbors grid nodes i - T_neighbors + 1, i - T_neighbors + 2, ..., i + T_neighbors.
     ...
     * for the forward derivative and where D_x is the derivative operator along x, dx the grid spacing along x, and
     * g_l^{2T_neighbors} weightings for the finite differences of different distance l from the point i
     * of computation.
     *
     * @tparam T_neighbors Number of neighbors used to calculate
     *                     the derivative from finite differences.
     ...
     */
     template<
        ...
        uint32_t T_direction
     >
     using ForwardDerivativeFunctor = detail::GeneralAofdtdDerivative<
         T_neighbors - 1,
         T_neighbors,
         T_neighbors,
         T_direction
    ...
    template<
        ...
        uint32_t T_direction
    >
    using BackwardDerivativeFunctor = detail::GeneralAofdtdDerivative<
        T_neighbors,
        T_neighbors - 1,
        T_neighbors,
        T_direction
...
namespace traits
{
    ...
    struct DerivativeFunctor<
        maxwellSolver::aoFDTD::Forward< T_neighbors >,
        T_direction
    > : pmacc::meta::accessors::Identity<
        maxwellSolver::aoFDTD::ForwardDerivativeFunctor<
            T_neighbors,
            T_direction
        >
    >
    ...
    >
    struct DerivativeFunctor<
        maxwellSolver::aoFDTD::Backward< T_neighbors >,
        T_direction
    > : pmacc::meta::accessors::Identity<
        maxwellSolver::aoFDTD::BackwardDerivativeFunctor<
            T_neighbors,
            T_direction
        >
    >
