            ...
            namespace profiles
            {
                ...
                private:
                    OpenPMDdata()
                    {
                        ...
                        if(it_prop != std::end(axisLabels))
                            ...
                            throw std::runtime_error(
                                "Error: could not find propagation axis " + std::string(Params::propagationAxisOpenPMD)
                                + " in OpenPMD dataset");
                                ...
                                for(uint32_t i = 0; i < extentOpenPMD[0]; i++)
                                {
                                    for(uint32_t j = 0; j < extentOpenPMD[1]; j++)
                                    {
                                        for(uint32_t k = 0; k < extentOpenPMD[2]; k++)
                                        {
                                            ...
                                            {
                                                valLeft = pmacc::math::abs(hostFieldDataBox(DataSpace<3u>(i, j, k)));
                                                if(valLeft > maxEDiscarded) // left discarded area
                                                    maxEDiscarded = valLeft;
                                                valRight = pmacc::math::abs(
                                                    hostFieldDataBox(DataSpace<3u>(extentOpenPMD[d] - 1 - i, j, k)));
                                                       ...
                                                       pmacc::math::abs(dataBoxOffset(d)) / dataBoxCellSize(d)))
                                            {
                                                ...
                                                if(valLeft > maxEDiscarded) // left discarded area
                                                    maxEDiscarded = valLeft;
                                                valRight = pmacc::math::abs(
                                                    hostFieldDataBox(DataSpace<3u>(i, extentOpenPMD[d] - 1 - j, k)));
                                                if(valRight > maxEDiscarded) // right discarded area
                                                    maxEDiscarded = valRight;
                                            }
                                            if(d == 2
                                                       ...
                                                       pmacc::math::abs(dataBoxOffset(d)) / dataBoxCellSize(d)))
                                            {
                                                ...
                                                if(valLeft > maxEDiscarded) // left discarded area
                                                    maxEDiscarded = valLeft;
                                                valRight = pmacc::math::abs(
                                                    hostFieldDataBox(DataSpace<3u>(i, j, extentOpenPMD[d] - 1 - k)));
                                                if(valRight > maxEDiscarded) // right discarded area
                                                    maxEDiscarded = valRight;
                                            }
                                        } // k
                ...
                private:
                        ...
                        float3_X const& pos) const
                    {
                        ...
                        dataInterp += dataBox(idxClosest) * (float3_X::create(1.0_X) - weight).productOfComponents();
                        for(uint32_t d = 0u; d < 3u; d++)
                        {
                            idxNext = idxClosest;
