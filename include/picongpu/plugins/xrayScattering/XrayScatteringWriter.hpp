...
private:
        ...
        HINLINE void openSeries( ::openPMD::AccessType at )
        {
            if( !openPMDSeries )
            {
                ...
                log< picLog::INPUT_OUTPUT >(
                    "XrayScatteringWriter: Successfully opened file: %1%" )
                    % fullName;
            }
            else
            {
                throw std::runtime_error(
                    "XrayScatteringWriter: Tried opening a Series while old "
        ...
        HINLINE void closeSeries( )
        {
            if( openPMDSeries )
            {
                ...
                log< picLog::INPUT_OUTPUT >(
                    "XrayScatteringWriter: successfully closed file: %1%" ) %
                fileName;
            }
            else
            {
                throw std::runtime_error(
                    "XrayScatteringWriter: Tried closing a Series that was not"
                    " active." );
            }
        }
...
public:
        ...
        )
        {
            ...
            uint64_t numFullLines = ( extent1D - firstLineLength ) /
                globalExtent[ 1 ];
            ...
            uint64_t lastLineLength( (extent1D - firstLineLength
                - numFullLines * globalExtent[ 1 ] ) );
            if ( lastLineLength != 0 )
            {
                localOffset = firstLineLength + numFullLines  *
                    globalExtent[ 1 ];
                offset = map2d(
                    globalExtent,
                    offset1D + localOffset
