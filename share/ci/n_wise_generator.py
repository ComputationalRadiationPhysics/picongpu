...
def is_valid_combination(row):
    ...
    if n >= 2:
        ...
        # clang_cuda images can currenlty not be used because
        # the base image is hard coding -DALPAKA_CUDA_COMPILER=clang
        if is_nvcc and is_clang:
            return False

        # hipcc is only valid in one combination
        if is_hip and is_hipcc and is_clang and v_compiler == 11:
            return True
