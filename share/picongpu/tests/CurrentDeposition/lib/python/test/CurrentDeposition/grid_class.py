...
class grid:
    ...
    def particle_step(self, pos1, pos2, pos_off1, pos_off2):
        ...
        start_x_cell = int((self.num_cells - 1) / 2)
        start_y_cell = int((self.num_cells - 1) / 2)
        start_z_cell = int((self.num_cells - 1) / 2)
        ...
        self.sim_add_x = min(pos_off1[0], pos_off2[0])
        self.sim_add_y = min(pos_off1[1], pos_off2[1])
        self.sim_add_z = min(pos_off1[2], pos_off2[2])
    ...
    def current_density_field(self, W_grid_x, W_grid_y, W_grid_z, start_coord,
        ...
        # determine start x
        start = np.min([start_coord-half_width, end_coord-half_width],
                       axis=0).astype(int)

        # determine end x
        end = np.min([start_coord+half_width+1, end_coord+half_width+1],
                     axis=0).astype(int)
        ...
        r_i = self.sim_add_x - start[0] - 1
        r_j = self.sim_add_y - start[1] - 1
        r_k = self.sim_add_z - start[2] - 1
