...
def get_params(path):
    ...
    if len(series.iterations) == 1:
        ...
        raise ValueError("There is more than one particle in the series "
...
def compare(j_grid_x, j_grid_y, j_grid_z, J):
    ...
    if (np.all(x_compare < epsilon) and np.all(y_compare < epsilon) and
            np.all(z_compare < epsilon)):
...
def main(dataPath):
    ...
    start_koord, end_koord = compare_grid.particle_step(poss[0], poss[1],
                                                        pos_offs[0],
                                                        pos_offs[1])
    ...
    j_grid_x, j_grid_y, j_grid_z = compare_grid.current_density_field(
        W_grid_x, W_grid_y, W_grid_z, start_koord, end_koord, charge[0],
        params, shape)
