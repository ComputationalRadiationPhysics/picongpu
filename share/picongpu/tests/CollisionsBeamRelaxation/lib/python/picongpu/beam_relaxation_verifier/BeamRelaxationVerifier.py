...
class BeamRelaxationVerifier:
    def __init__(self, sim_output_path):
        ...
        self.series = api.Series(str(sim_path.resolve()),
        ...
        self.debug_values = {
            ratio: _get_debug_data(sim_output_path, collider_id_map[ratio]) for
            ratio in self.ratios}
    ...
    def calculate_values(self, n_cells=None):
        ...
        for i, it in enumerate(self.series.iterations):
            ...
            species_prefix = {'equal': {'e': 'e_1000', 'i': 'i_1000'},
                              'moreIons': {'e': 'e_100', 'i': 'i_1000'},
                              'lessIons': {'e': 'e_1000', 'i': 'i_100'}}

            x_cell_range = {'equal': (0, 32), 'lessIons': (32, 64),
                            'moreIons': (64, 96)}
            for ratio in self.ratios:
                ...
                local_array_dict['e_vperp'][ratio] = (
                    e_vperp_mrc[:, x_cell_range[ratio][0]:
                                x_cell_range[ratio][1]])
                e_vx_mrc = iteration.meshes[
                    species_prefix[ratio]['e'] + '_' + vx_field_name][
                    api.Mesh_Record_Component.SCALAR]
                local_array_dict['e_vx'][ratio] = (
                    e_vx_mrc[:, x_cell_range[ratio][0]:
                             x_cell_range[ratio][1]])
                i_vx_mrc = iteration.meshes[
                    species_prefix[ratio]['i'] + '_' + vx_field_name][
                    api.Mesh_Record_Component.SCALAR]
                local_array_dict['i_vx'][ratio] = (
                    i_vx_mrc[:, x_cell_range[ratio][0]:
                             x_cell_range[ratio][1]])
            ...
            if n_cells is not None:
                for quantity, ratio in itertools.product(self.quantities,
                                                         self.ratios):
                    local_array_dict[quantity][ratio] = np.ravel(
                        local_array_dict[quantity][ratio])[0:n_cells]
            ...
            for quantity, ratio in itertools.product(self.quantities,
                ...
                self.calculated_data['mean'][quantity][ratio][i] = np.average(
                    data)
                self.calculated_data['std_mean'][quantity][ratio][i] = np.std(
                    data) / np.sqrt(data.size)
                self.calculated_data['std_dist'][quantity][ratio][i] = np.std(
                    data)

    def plot(self, to_file=False, file_name=None, smilei_loader=None):
        times = np.array(self.series.iterations) * self.dt * 1e15
        f, ax_dict = plt.subplot_mosaic([['equal_std_mean', 'equal_std_mean',
                                          'moreIons_std_mean',
                                          'moreIons_std_mean',
                                          'lessIons_std_mean',
                                          'lessIons_std_mean'],
                                         ['equal_std_dist', 'equal_std_dist',
                                          'moreIons_std_dist',
                                          'moreIons_std_dist',
                                          'lessIons_std_dist',
                                          'lessIons_std_dist'],
                                         ['all_pic', 'all_pic', 'all_pic',
                                          'all_smilei', 'all_smilei',
                                          'all_smilei']],
                                        sharey=True,
                                        gridspec_kw={
                                            'height_ratios': [1, 1, 1]},
                                        figsize=(20, 22))
        label_map = {'e_vperp': r'$v_{e, \perp}$', 'e_vx': r'$v_{e,x}$',
                     'i_vx': r'$v_{i,x}$'}
        for std_type in ['std_mean', 'std_dist']:
            for ratio in self.ratios:
                for quantity in self.quantities:
                    ...
                    plot_with_std(ax,
                                  times,
                                  self.calculated_data['mean'][quantity][
                                      ratio] / cs.c,
                                  self.calculated_data[std_type][quantity][
                                      ratio] / cs.c,
                                  label=label_map[quantity] + ' PIConGPU')
                    if smilei_loader is not None:
                        plot_with_std(ax,
                                      smilei_loader.times_dict[ratio],
                                      smilei_loader.calculated_data['mean'][
                                          quantity][ratio],
                                      smilei_loader.calculated_data[std_type][
                                          quantity][ratio],
        ...
        for quantity in self.quantities:
            ...
            for ratio in self.ratios:
                ...
                ax_dict['all_pic'].plot(times,
                                        self.calculated_data['mean'][quantity][
                                            ratio] / cs.c,
                                        label=(label_map[quantity] +
                                               ' ' + ratio),
                                        color=color,
                                        ls=line_style)
                if smilei_loader is not None:
                    ax_dict['all_smilei'].plot(smilei_loader.times_dict[ratio],
                                               smilei_loader.calculated_data[
                                                   'mean'][quantity][ratio],
                                               label=(label_map[quantity]
                                                      + ' ' + ratio),
                                               color=color,
    ...
    def plot_debug_values(self,
        ...
        for ratio in self.ratios:
            ...
            axs[0].plot(times,
                        ...
                        label=fr'$\ln \Lambda$ PIConGPU {ratio}',
                        color='red',
                        ls=line_style,
                        alpha=0.75)
            axs[1].plot(times,
                        self.debug_values[ratio]['s_param'],
                        label=fr'$s$ PIConGPU {ratio}',
                        color='red',
                        ls=line_style,
                        alpha=0.75)
            if smilei_loader is not None:
                times_smilei = smilei_loader.times_dict[ratio][:-1]
                axs[0].plot(times_smilei,
                            smilei_loader.debug_values[ratio]['coulomb_log'],
                            label=fr'$\ln \Lambda$ smilei {ratio}',
                            color='black',
                            ls=line_style,
                            alpha=0.75)
                axs[1].plot(times_smilei,
                            smilei_loader.debug_values[ratio]['s_param'],
                            label=fr'$s$ smilei {ratio}',
                            color='black',
                            ls=line_style,
                            alpha=0.75)
                axs[2].plot(times_smilei,
                            smilei_loader.debug_values[ratio]['debye_length'],
                            label=fr'$\lambda_D$ smilei {ratio}',
                            color='black',
                            ls=line_style,
                            alpha=0.75)
