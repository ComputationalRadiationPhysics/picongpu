...
class SmileiBeamRelaxation:
    def __init__(self, path_equal, path_less_ions, path_more_ions):
        ...
        def get_iterator():
            return zip(self.ratios, [path_equal, path_less_ions,
                                     path_more_ions])

        self.sims = {key: happi.Open(path) for key, path in get_iterator()}
        self.unit_length = (cs.c / self.sims[
            self.ratios[0]].namelist.Main.reference_angular_frequency_SI)
        self.debug_values = {key: self._load_debug_output(path) for key, path
                             in get_iterator()}

        def get_times(sim):
            return sim.ParticleBinning(diagNumber=0, units=['fs']).getTimes()
        self.times_dict = {key: get_times(sim) for key, sim in
                           self.sims.items()}
            ...
            for quantity in self.quantities:
                ...
                for ratio in self.ratios:
                    self.calculated_data[key][quantity][ratio] = np.zeros_like(
                        self.times_dict[ratio],
                        dtype=np.float64)

    def _calculate_values(self, ratio):
        sim = self.sims[ratio]
        w_vx_e = sim.ParticleBinning(0, ).get()
        ...
        for i, t in enumerate(self.times_dict[ratio]):
            local_array_dict = {'e_vx': w_vx_e["data"][i] / n_e["data"][i],
                                'i_vx': w_vx_i["data"][i] / n_i["data"][i],
                                'e_vperp': np.sqrt(
                                    w_vperp2_e["data"][i] / n_e["data"][i])}
    ...
    def _load_debug_output(self, path):
        ...
        for ii, iteration in enumerate(list(debug_output_file)):
            debye_length[ii] = np.average(
                debug_output_file[iteration]['debyelength'])
            coulomb_log[ii] = np.average(
                debug_output_file[iteration]['coulomb_log'])
            s_param[ii] = np.average(debug_output_file[iteration]['s'])
        output = np.empty((n_iterations,),
                          dtype=[('coulomb_log', np.float64),
                                 ('s_param', np.float64),
