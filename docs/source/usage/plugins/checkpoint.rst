...
For further details, see the according section in :ref:`the openPMD API <usage-plugins-openPMD>`.

External Dependencies
^^^^^^^^^^^^^^^^^^^^^

The plugin is available as soon as the :ref:`openPMD API library <install-dependencies>` are compiled in.
