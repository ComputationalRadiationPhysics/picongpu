...
def get_sim_cfl_helper(
        ...
        method: str, n: int = 100) -> picmi.Simulation:
...
class TestPicmiSimulation(unittest.TestCase):
    ...
    def test_species_translation(self):
        ...
        sim.add_species(picmi.Species(name="dummy2",
                                      mass=3,
                                      density_scale=4,
        ...
        sim.add_species(picmi.Species(name="dummy3",
                                      mass=3,
                                      initial_distribution=profile),
                        layout)
        ...
        species_names = set(map(lambda species: species.name,
                                picongpu.init_manager.all_species))
        self.assertEqual({"dummy1", "dummy2", "dummy3"},
                         species_names)
    ...
    def test_operations_simple_density_translated(self):
        ...
        self.sim.add_species(picmi.Species(name="colocated1",
                                           mass=1,
                                           density_scale=4,
                                           initial_distribution=profile),
                             layout)
        self.sim.add_species(picmi.Species(name="colocated2",
                                           mass=2,
                                           density_scale=2,
                                           initial_distribution=profile),
                             layout)
        self.sim.add_species(picmi.Species(name="separate1",
                                           mass=3,
                                           initial_distribution=other_profile),
                             layout)
        self.sim.add_species(picmi.Species(name="separate2",
                                           mass=4,
                                           initial_distribution=profile),
                             other_layout)
        ...
        self.assertEqual(
            ...
            list(map(lambda species: species.name,
                     initmgr.all_species)))

        # operations
        density_operations = list(filter(
        ...
        for op in density_operations:
            self.assertTrue(
                isinstance(op.profile,
                           species.operation.densityprofile.Uniform))
    ...
    def test_operation_not_placed_translated(self):
        ...
        self.sim.add_species(picmi.Species(name="notplaced",
    ...
    def test_operation_momentum(self):
        ...
        self.sim.add_species(
            picmi.Species(name="valid",
                          mass=17,
                          initial_distribution=picmi.UniformDistribution(
                              density=17,
                              rms_velocity=[17, 17, 17],
                              directed_velocity=[31283745.0,
                                                 45132121.0,
                                                 215484563.0]
                          )),
        ...
        self.assertAlmostEqual(3.06645343e19,
                               mom_op.temperature.temperature_kev,
                               delta=1e13)
        self.assertEqual(mom_op.drift.direction_normalized,
                         (0.14068221552237223,
                          0.2029580145696681,
                          0.9690286675623457))
        self.assertAlmostEqual(1.491037242289643,
                               mom_op.drift.gamma)
    ...
    def test_ionization_electron_explicit(self):
        ...
        electron_logs = list(filter(lambda line: "electron" in line,
                                    caught_logs.output))
        self.assertEqual([], electron_logs)

        # ensure species actually exists
        pypic_species_by_name = dict(
            map(lambda species: (species.name, species),
                pypic_sim.init_manager.all_species))
        self.assertEqual({"e1", "e2", "ion"},
                         set(pypic_species_by_name.keys()))

        pypic_ion = pypic_species_by_name["ion"]
        self.assertTrue(pypic_ion.has_constant_of_type(
                species.constant.Ionizers))

        ionizers = pypic_ion.get_constant_by_type(
            species.constant.Ionizers)

        # relationship preserved:
        self.assertTrue(
            ionizers.electron_species is pypic_species_by_name["e2"])
    ...
    def test_ionization_electron_resolution_added(self):
        ...
        ##
        hydrogen = picmi.Species(name="hydrogen",
                                 particle_type="H",
                                 charge_state=+1,
                                 initial_distribution=profile)
        sim = self.__get_sim()
    ...
    def test_ionization_electron_resolution_guessed(self):
        ...
        for electron_explicit in [True, False]:
            ...
            if electron_explicit:
                ...
                electron = picmi.Species(name="my_e",
            ...
            for pypic_species in pypic_sim.init_manager.all_species:
                ...
                ionizers_const = pypic_species.get_constant_by_type(
                    species.constant.Ionizers)
    ...
    def test_ionization_electron_resolution_guess_ambiguous(self):
        ...
        e1 = picmi.Species(name="the_first_electrons",
                           particle_type="electron")
        e2 = picmi.Species(name="the_other_electrons",
                           mass=picmi.constants.m_e,
                           charge=-picmi.constants.q_e)
        profile = picmi.UniformDistribution(7)
        helium = picmi.Species(name="helium",
                               particle_type="He",
                               charge_state=+1,
                               initial_distribution=profile)
    ...
    def test_ionization_electron_not_added(self):
        ...
        with self.assertRaisesRegex(
                AssertionError,
                ".*my_e.*helium.*picongpu_ionization_species.*"):
            sim.get_as_pypicongpu()
    ...
    def test_ionization_added_electron_namecollision(self):
        ...
        for name in existing_electron_names:
            self.sim.add_species(
                picmi.Species(name=name, mass=1, charge=1),
                None)
        ...
        self.assertEqual(1 + len(existing_electron_names + ["ion"]),
    ...
    def test_ionization_methods_added(self):
        ...
        ion1 = picmi.Species(name="hydrogen",
                             particle_type="H",
                             charge_state=+1)
        ion2 = picmi.Species(name="nitrogen",
                             particle_type="N",
                             charge_state=+2)
        ...
        operation_types = list(map(lambda op: type(op),
                                   initmgr.all_operations))
        self.assertEqual(1, operation_types.count(
