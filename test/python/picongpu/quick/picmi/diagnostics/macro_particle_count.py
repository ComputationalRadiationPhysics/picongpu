...
TESTCASES_VALID = [
    (
        ...
        {"species": {"name": "electron"}, "period": {"specs": [{"start": 0, "stop": -1, "step": 10}]}},
    ),
    ({"species": PICMISpecies(name="proton"), "period": 0}, {"species": {"name": "proton"}, "period": {"specs": []}}),
    (
        {"species": PICMISpecies(name="ion"), "period": TimeStepSpec[::10]},
        {"species": {"name": "ion"}, "period": {"specs": [{"start": 0, "stop": -1, "step": 10}]}},
    ),
    (
    ...
    (
        {"species": PICMISpecies(name="proton"), "period": TimeStepSpec[-10:]},
        {"species": {"name": "proton"}, "period": {"specs": [{"start": 90, "stop": 99, "step": 1}]}},
    ),
    (
...
TESTCASES_INVALID_TIMESTEPS = [
    ({"species": PICMISpecies(name="electron"), "period": TimeStepSpec[::-10]}, "Step size must be >= 1"),
]
...
class TestMacroParticleCount(unittest.TestCase):
            ...
            with self.subTest(params=params):
                ...
                for key, value in params.items():
                    if key == "period" and isinstance(value, int):
                        ...
                        self.assertEqual(
                            macro_count.period.get_as_pypicongpu(0.5, 100).get_rendering_context(),
    ...
    def test_macroparticlecount_serialization(self):
        ...
        for params, expected_serialized in TESTCASES_VALID:
            with self.subTest(params=params, expected_serialized=expected_serialized):
                macro_count = MacroParticleCount(**params)
                pypicongpu_macro_count = macro_count.get_as_pypicongpu(species_map, 0.5, 100)
    ...
    def test_macroparticlecount_invalid_species(self):
        ...
        with self.assertRaisesRegex(ValueError, "Species " "unknown" " is not known to Simulation"):
            macro_count.get_as_pypicongpu({}, 0.5, 100)
        ...
        for params, expected_error in TESTCASES_INVALID_TIMESTEPS:
            with self.subTest(params=params, expected_error=expected_error):
                ...
                with self.assertRaisesRegex(ValueError, expected_error):
                    macro_count.get_as_pypicongpu(species_map, 0.5, 100)
    ...
    def test_macroparticlecount_invalid_simulation_parameters(self):
        ...
        with self.assertRaisesRegex(ValueError, "Time step size must be strictly positive"):
            macro_count.get_as_pypicongpu(species_map, -0.5, 100)
        with self.assertRaisesRegex(ValueError, "Time step size must be strictly positive"):
            macro_count.get_as_pypicongpu(species_map, 0, 100)
