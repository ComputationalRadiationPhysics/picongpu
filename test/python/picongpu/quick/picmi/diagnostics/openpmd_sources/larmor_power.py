...
from picongpu.picmi.diagnostics.openpmd_sources import LarmorPower
...
class PICMI_TestLarmorPower(unittest.TestCase):
    def test_larmor_power_instantiation(self):
        """Test LarmorPower instantiation and validation."""
        for params, _ in TESTCASES_VALID:
            with self.subTest(params=params):
                source = LarmorPower(**params)
    ...
    def test_larmor_power_serialization(self):
        """Test LarmorPower serialization to PyPIConGPULarmorPower."""
        for params, expected_serialized in TESTCASES_VALID:
            with self.subTest(params=params, expected_serialized=expected_serialized):
                ...
                self.assertIsInstance(pypicongpu_source, PyPIConGPULarmorPower)
    ...
    def test_larmor_power_invalid_mapping(self):
        """Test LarmorPower with invalid species mapping."""
        for params, expected_error in TESTCASES_INVALID_MAPPING:
            with self.subTest(params=params, expected_error=expected_error):
                source = LarmorPower(species=params["species"], filter=params["filter"])
