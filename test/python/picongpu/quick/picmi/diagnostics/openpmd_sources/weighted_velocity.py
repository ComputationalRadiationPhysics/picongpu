...
from picongpu.picmi.diagnostics.openpmd_sources import WeightedVelocity
from picongpu.pypicongpu.output.openpmd_sources import WeightedVelocity as PyPIConGPUWeightedVelocity
...
class PICMI_TestWeightedVelocity(unittest.TestCase):
    def test_weighted_velocity_instantiation(self):
        """Test WeightedVelocity instantiation and validation."""
        for params, _ in TESTCASES_VALID:
            with self.subTest(params=params):
                source = WeightedVelocity(**params)
        ...
        for params, expected_error in TESTCASES_INVALID:
            with self.subTest(params=params, expected_error=expected_error):
                with self.assertRaisesRegex((ValueError, typeguard.TypeCheckError), expected_error):
                    WeightedVelocity(**params)

    def test_weighted_velocity_serialization(self):
        """Test WeightedVelocity serialization to PyPIConGPUWeightedVelocity."""
        for params, expected_serialized in TESTCASES_VALID:
            with self.subTest(params=params, expected_serialized=expected_serialized):
                ...
                self.assertIsInstance(pypicongpu_source, PyPIConGPUWeightedVelocity)
    ...
    def test_weighted_velocity_invalid_mapping(self):
        """Test WeightedVelocity with invalid species mapping."""
        for params, expected_error in TESTCASES_INVALID_MAPPING:
            with self.subTest(params=params, expected_error=expected_error):
                source = WeightedVelocity(
