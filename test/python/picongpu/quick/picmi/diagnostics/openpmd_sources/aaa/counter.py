...
from picongpu.picmi.diagnostics.openpmd_sources import Counter
...
# Test cases for valid Counter inputs
TESTCASES_VALID = [
    (
...
class PICMI_TestCounter(unittest.TestCase):
    def test_counter_instantiation(self):
        """Test Counter instantiation and validation."""
        for params, _ in TESTCASES_VALID:
            with self.subTest(params=params):
                source = Counter(**params)
    ...
    def test_counter_serialization(self):
        """Test Counter serialization to PyPIConGPUCounter."""
        for params, expected_serialized in TESTCASES_VALID:
            with self.subTest(params=params, expected_serialized=expected_serialized):
                ...
                self.assertIsInstance(pypicongpu_source, PyPIConGPUCounter)
    ...
    def test_counter_invalid_mapping(self):
        """Test Counter with invalid species mapping."""
        for params, expected_error in TESTCASES_INVALID_MAPPING:
            with self.subTest(params=params, expected_error=expected_error):
                source = Counter(species=params["species"], filter=params["filter"])
                with self.assertRaisesRegex(ValueError, expected_error):
