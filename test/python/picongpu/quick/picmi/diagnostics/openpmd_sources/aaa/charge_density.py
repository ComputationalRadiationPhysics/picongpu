...
from picongpu.picmi.diagnostics.openpmd_sources import ChargeDensity
...
# Test cases for valid ChargeDensity inputs
TESTCASES_VALID = [
    (
...
class PICMI_TestChargeDensity(unittest.TestCase):
    def test_charge_density_instantiation(self):
        """Test ChargeDensity instantiation and validation."""
        for params, _ in TESTCASES_VALID:
            with self.subTest(params=params):
                source = ChargeDensity(**params)
    ...
    def test_charge_density_serialization(self):
        """Test ChargeDensity serialization to PyPIConGPUChargeDensity."""
        for params, expected_serialized in TESTCASES_VALID:
            with self.subTest(params=params, expected_serialized=expected_serialized):
                ...
                self.assertIsInstance(pypicongpu_source, PyPIConGPUChargeDensity)
    ...
    def test_charge_density_invalid_mapping(self):
        """Test ChargeDensity with invalid species mapping."""
        for params, expected_error in TESTCASES_INVALID_MAPPING:
            with self.subTest(params=params, expected_error=expected_error):
                source = ChargeDensity(species=params["species"], filter=params["filter"])
                with self.assertRaisesRegex(ValueError, expected_error):
