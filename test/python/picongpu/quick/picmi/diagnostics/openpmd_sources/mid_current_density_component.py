...
from picongpu.picmi.diagnostics.openpmd_sources import MidCurrentDensityComponent
...
class PICMI_TestMidCurrentDensityComponent(unittest.TestCase):
    def test_mid_current_density_component_instantiation(self):
        """Test MidCurrentDensityComponent instantiation and validation."""
        for params, _ in TESTCASES_VALID:
            with self.subTest(params=params):
                source = MidCurrentDensityComponent(**params)
        ...
        for params, expected_error in TESTCASES_INVALID:
            with self.subTest(params=params, expected_error=expected_error):
                with self.assertRaisesRegex((ValueError, typeguard.TypeCheckError), expected_error):
                    MidCurrentDensityComponent(**params)

    def test_mid_current_density_component_serialization(self):
        """Test MidCurrentDensityComponent serialization to PyPIConGPUMidCurrentDensityComponent."""
        for params, expected_serialized in TESTCASES_VALID:
            with self.subTest(params=params, expected_serialized=expected_serialized):
                ...
                self.assertIsInstance(pypicongpu_source, PyPIConGPUMidCurrentDensityComponent)
    ...
    def test_mid_current_density_component_invalid_mapping(self):
        """Test MidCurrentDensityComponent with invalid species mapping."""
        for params, expected_error in TESTCASES_INVALID_MAPPING:
            with self.subTest(params=params, expected_error=expected_error):
                source = MidCurrentDensityComponent(
