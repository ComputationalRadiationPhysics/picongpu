...
from picongpu.picmi.diagnostics.openpmd_sources import Momentum
...
class PICMI_TestMomentum(unittest.TestCase):
    def test_momentum_instantiation(self):
        """Test Momentum instantiation and validation."""
        for params, _ in TESTCASES_VALID:
            with self.subTest(params=params):
                source = Momentum(**params)
    ...
    def test_momentum_serialization(self):
        """Test Momentum serialization to PyPIConGPUMomentum."""
        for params, expected_serialized in TESTCASES_VALID:
            with self.subTest(params=params, expected_serialized=expected_serialized):
                ...
                self.assertIsInstance(pypicongpu_source, PyPIConGPUMomentum)
    ...
    def test_momentum_invalid_mapping(self):
        """Test Momentum with invalid species mapping."""
        for params, expected_error in TESTCASES_INVALID_MAPPING:
            with self.subTest(params=params, expected_error=expected_error):
                source = Momentum(species=params["species"], filter=params["filter"], direction=params["direction"])
