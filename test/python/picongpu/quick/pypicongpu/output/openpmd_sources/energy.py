...
class TestEnergy(unittest.TestCase):
    def test_source_energy(self):
        ...
        source = Energy(species=MockSpecies(), filter="custom")
        self.assertEqual(source.filter, "custom")
        source.check()

        # Test invalid filter type
        with self.assertRaises(ValueError):
            Energy(species=MockSpecies(), filter=123).check()

        # Test invalid species type
        with self.assertRaises(ValueError):
            Energy(species="invalid").check()

        # Test serialization
        openpmd = OpenPMD(
            period=TimeStepSpec([slice(0, None, 100)]), source=[Energy(species=MockSpecies(), filter="custom")]
        )
