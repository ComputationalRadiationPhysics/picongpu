...
from picongpu.pypicongpu.output.openpmd_sources import Energy
...
class TestEnergy(unittest.TestCase):
    def test_source_energy(self):
        """Test Energy instantiation and serialization."""
        ...
        source = Energy(species=MockSpecies(), filter="custom_filter")
        self.assertEqual(source.filter, "custom_filter")
        source.check()

        source = Energy(species=MockSpecies(), filter="fields_all")
        ...
        openpmd = OpenPMD(period=TimeStepSpec([slice(0, None, 100)]), source=[Energy(species=MockSpecies())])
        context = openpmd.get_rendering_context()
        self.assertEqual(context["data"]["source"][0]["type"], "energy")
