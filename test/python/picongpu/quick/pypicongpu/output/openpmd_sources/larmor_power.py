...
from picongpu.pypicongpu.output.openpmd_sources import LarmorPower
...
class TestLarmorPower(unittest.TestCase):
    def test_source_larmor_power(self):
        """Test LarmorPower instantiation and serialization."""
        ...
        source = LarmorPower(species=MockSpecies(), filter="custom_filter")
        self.assertEqual(source.filter, "custom_filter")
        source.check()

        source = LarmorPower(species=MockSpecies(), filter="fields_all")
        ...
        self.assertEqual(context["source"][0]["type"], "larmorpower")
        ...
        openpmd = OpenPMD(period=TimeStepSpec([slice(0, None, 100)]), source=[LarmorPower(species=MockSpecies())])
        context = openpmd.get_rendering_context()
        self.assertEqual(context["data"]["source"][0]["type"], "larmorpower")
