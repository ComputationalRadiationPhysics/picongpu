...
class TestLarmorPower(unittest.TestCase):
    def test_source_larmor_power(self):
        ...
        source = LarmorPower(species=MockSpecies(), filter="custom")
        self.assertEqual(source.filter, "custom")
        source.check()

        # Test invalid filter type
        with self.assertRaises(ValueError):
            LarmorPower(species=MockSpecies(), filter=123).check()

        # Test invalid species type
        with self.assertRaises(ValueError):
            LarmorPower(species="invalid").check()

        # Test serialization
        openpmd = OpenPMD(
            period=TimeStepSpec([slice(0, None, 100)]), source=[LarmorPower(species=MockSpecies(), filter="custom")]
        )
