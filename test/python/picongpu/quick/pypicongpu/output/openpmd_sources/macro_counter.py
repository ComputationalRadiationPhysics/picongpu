...
class TestMacroCounter(unittest.TestCase):
    def test_source_macro_counter(self):
        ...
        source = MacroCounter(species=MockSpecies(), filter="custom")
        self.assertEqual(source.filter, "custom")
        source.check()

        # Test invalid filter type
        with self.assertRaises(ValueError):
            MacroCounter(species=MockSpecies(), filter=123).check()

        # Test invalid species type
        with self.assertRaises(ValueError):
            MacroCounter(species="invalid").check()

        # Test serialization
        openpmd = OpenPMD(
            period=TimeStepSpec([slice(0, None, 100)]), source=[MacroCounter(species=MockSpecies(), filter="custom")]
        )
