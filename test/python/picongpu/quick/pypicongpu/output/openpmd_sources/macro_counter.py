...
from picongpu.pypicongpu.output.openpmd_sources import MacroCounter
...
class TestMacroCounter(unittest.TestCase):
    def test_source_macro_counter(self):
        """Test MacroCounter instantiation and serialization."""
        ...
        source = MacroCounter(species=MockSpecies(), filter="custom_filter")
        self.assertEqual(source.filter, "custom_filter")
        source.check()

        source = MacroCounter(species=MockSpecies(), filter="fields_all")
        ...
        self.assertEqual(context["source"][0]["type"], "macrocounter")
        ...
        openpmd = OpenPMD(period=TimeStepSpec([slice(0, None, 100)]), source=[MacroCounter(species=MockSpecies())])
        context = openpmd.get_rendering_context()
        self.assertEqual(context["data"]["source"][0]["type"], "macrocounter")
