...
class TestCounter(unittest.TestCase):
    def test_source_counter(self):
        ...
        source = Counter(species=MockSpecies(), filter="custom")
        self.assertEqual(source.filter, "custom")
        source.check()

        # Test invalid filter type
        with self.assertRaises(ValueError):
            Counter(species=MockSpecies(), filter=123).check()

        # Test invalid species type
        with self.assertRaises(ValueError):
            Counter(species="invalid").check()

        # Test serialization
        openpmd = OpenPMD(
            period=TimeStepSpec([slice(0, None, 100)]), source=[Counter(species=MockSpecies(), filter="custom")]
        )
