...
from picongpu.pypicongpu.output.openpmd_sources import WeightedVelocity
...
class TestWeightedVelocity(unittest.TestCase):
    def test_source_weighted_velocity(self):
        ...
        source = WeightedVelocity(species=MockSpecies(), filter="custom_filter", direction="y")
        self.assertEqual(source.filter, "custom_filter")
        self.assertEqual(source.direction, "y")
        source.check()

        source = WeightedVelocity(species=MockSpecies(), filter="fields_all", direction="z")
        ...
        with self.assertRaisesRegex(
            ...
            WeightedVelocity(species=MockSpecies(), filter="invalid").check()

        with self.assertRaisesRegex(typeguard.TypeCheckError, r"argument \"filter\" \(int\) is not an instance of str"):
            WeightedVelocity(species=MockSpecies(), filter=123)
        ...
        openpmd = OpenPMD(
            ...
            source=[WeightedVelocity(species=MockSpecies(), filter="custom_filter", direction="z")],
        )
        context = openpmd.get_rendering_context()
        self.assertTrue(context["typeID"]["openpmd"])
        context = context["data"]
        self.assertEqual(len(context["source"]), 1)
        self.assertEqual(context["source"][0]["type"], "weightedvelocity")
        ...
        context = openpmd.get_rendering_context()
        self.assertEqual(context["data"]["source"][0]["type"], "weightedvelocity")
