...
from picongpu.pypicongpu.output.openpmd_sources import Density
...
class TestDensity(unittest.TestCase):
    def test_source_density(self):
        ...
        source = Density(species=MockSpecies(), filter="custom_filter")
        self.assertEqual(source.filter, "custom_filter")
        source.check()

        source = Density(species=MockSpecies(), filter="fields_all")
        ...
        with self.assertRaisesRegex(
            ...
            Density(species=MockSpecies(), filter="invalid").check()

        with self.assertRaisesRegex(typeguard.TypeCheckError, r"argument \"filter\" \(int\) is not an instance of str"):
            Density(species=MockSpecies(), filter=123)

        with self.assertRaisesRegex(
            typeguard.TypeCheckError,
            r"argument \"species\" \(str\) is not an instance of picongpu.pypicongpu.species.species.Species",
        ):
            Density(species="invalid")

        openpmd = OpenPMD(
            period=TimeStepSpec([slice(0, None, 100)]), source=[Density(species=MockSpecies(), filter="custom_filter")]
        )
        ...
        self.assertEqual(context["source"][0]["type"], "density")
        ...
        context = openpmd.get_rendering_context()
        self.assertEqual(context["data"]["source"][0]["type"], "density")
