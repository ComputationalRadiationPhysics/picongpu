...
from picongpu.pypicongpu.output.openpmd_sources import EnergyDensity
...
class TestEnergyDensity(unittest.TestCase):
    def test_source_energy_density(self):
        """Test EnergyDensity instantiation and serialization."""
        ...
        source = EnergyDensity(species=MockSpecies(), filter="fields_all")
        self.assertEqual(source.filter, "fields_all")
        source.check()

        source = EnergyDensity(species=MockSpecies(), filter="custom_filter")
        ...
        self.assertEqual(context["source"][0]["type"], "energydensity")
        ...
        openpmd = OpenPMD(period=TimeStepSpec([slice(0, None, 100)]), source=[EnergyDensity(species=MockSpecies())])
        context = openpmd.get_rendering_context()
        self.assertEqual(context["data"]["source"][0]["type"], "energydensity")
