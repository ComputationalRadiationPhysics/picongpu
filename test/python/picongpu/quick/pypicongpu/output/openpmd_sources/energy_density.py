...
class TestEnergyDensity(unittest.TestCase):
    def test_source_energy_density(self):
        ...
        source = EnergyDensity(species=MockSpecies(), filter="custom")
        self.assertEqual(source.filter, "custom")
        source.check()

        # Test invalid filter type
        with self.assertRaises(ValueError):
            EnergyDensity(species=MockSpecies(), filter=123).check()

        # Test invalid species type
        with self.assertRaises(ValueError):
            EnergyDensity(species="invalid").check()

        # Test serialization
        openpmd = OpenPMD(
            period=TimeStepSpec([slice(0, None, 100)]), source=[EnergyDensity(species=MockSpecies(), filter="custom")]
        )
