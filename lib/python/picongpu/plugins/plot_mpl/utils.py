...
def get_different_colormaps(n):
    ...
    myCmaps = [None] * n
    # NOTE: this requires n <= 10
    # colors = plt.rcParams['axes.prop_cycle'].by_key()['color'][:n]

    # This allows arbitrary number of colors but colors are less well distinguishable
    colors = get_different_colors(n)

    # generate the colors for colormaps
    for i, c in enumerate(colors):
        myColor1 = colorConverter.to_rgba('white', alpha=1.0)
        myColor2 = colorConverter.to_rgba(c, alpha=1.0)
