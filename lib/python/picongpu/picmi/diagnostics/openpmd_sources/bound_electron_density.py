...
class BoundElectronDensity(SourceBase):
    """
    Represents a bound electron density data source for openPMD output in particle-in-cell simulations.

    This source calculates the density of bound electrons from a specified particle species, optionally
    filtered by a selection criterion.

    Parameters
    ----------
    species: PICMISpecies
        Particle species contributing to the bound electron density (e.g., ions).
    ...
    def get_as_pypicongpu(
        ...
        -------
        PyPIConGPUBoundElectronDensity
            A PyPIConGPU BoundElectronDensity instance with the same filter and species.
