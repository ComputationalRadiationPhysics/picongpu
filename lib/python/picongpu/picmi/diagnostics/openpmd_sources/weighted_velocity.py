...
from ....pypicongpu.output.openpmd_sources import WeightedVelocity as PyPIConGPUWeightedVelocity
...
class WeightedVelocity(SourceBase):
    """
    Weighted velocity component data source for openPMD output

    Derives a scalar field of a weighted velocity component (in m/s, direction x=0, y=1, z=2)
    ...
    @param species Particle species to calculate weighted velocity for (e.g., electrons, ions).
        Must have position, momentum, weighting, and mass attributes.
    @param filter Name of a filter to select particles. Default: "all".
    @param direction Velocity component direction (0=x, 1=y, 2=z).
    """
    ...
    def get_as_pypicongpu(
    ...
    ) -> PyPIConGPUWeightedVelocity:
        """
        Convert to a PyPIConGPU WeightedVelocity source.

        @param dict_species_picmi_to_pypicongpu Mapping of PICMI to PyPIConGPU species.
        @return A PyPIConGPU WeightedVelocity instance with the same filter, species, and direction.
        ...
        return PyPIConGPUWeightedVelocity(
            filter=self.filter,
