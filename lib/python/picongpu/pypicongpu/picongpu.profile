...
if [[ "$CURRENT_WORKING_DIRECTORY" =~ ".*$(dirname $PIC_PROFILE)/worktrees/.*" ]]; then
  export PICSRC="$CURRENT_WORKING_DIRECTORY"
else
  export PICSRC="$HOME/workspace/src/picongpu"
fi
...
export TBG_TPLFILE="~/profiles/mpirun.tpl"
