...
def picmi_generate_input(directory, laser, **kwargs):
    ...
    laser = picmi.FromLasyLaser(
        ...
        file_path=os.path.join(
            directory.name if isinstance(directory, tempfile.TemporaryDirectory) else directory, "test_laser.bp"
        ),
        lasyLaser=laser,
        **kwargs,
    )
    ...
    sim.write_input_file(
        os.path.join(directory.name if isinstance(directory, tempfile.TemporaryDirectory) else directory, "setup/")
    )


def assert_valid_input_file(directory):
    """validates the input files generated by picmi, specifically the incidentField.param."""
    file = open(
        os.path.join(
            directory.name if isinstance(directory, tempfile.TemporaryDirectory) else directory,
            "setup/include/picongpu/param/incidentField.param",
        ),
        "r",
    )
    ...
    required_lines = {
        ...
        " LaserProfile_0 ": "profiles::FromOpenPMDPulse<PyPIConGPUFromOpenPMDPulseParam_0>",
    }

    for line in lines:
        for key in required_lines:
            if key in line:
                if line.split(" = ")[-1] == required_lines[key] + ";\n":
                    required_lines[key] = ""
                elif required_lines[key] == "":
                    print(f"Warning: important key {key} found multiple times")

    missing_lines = []
    for key in required_lines:
        if required_lines[key] != "":
            missing_lines.append(key)

    assert len(missing_lines) == 0, "Not all required lines were found. The following parameters are missing: " + str(
        missing_lines
    )
