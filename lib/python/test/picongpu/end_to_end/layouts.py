...
LAYOUTS = {
    "oneposition_1": OnePosition(n_macroparticles_per_cell=1),
    "oneposition_2": OnePosition(n_macroparticles_per_cell=2),
    "oneposition_2_offset": OnePosition(n_macroparticles_per_cell=2, in_cell_offset=[0.1, 0.2, 0.3]),
        ...
        offsets = pd.concat(
            {
                ...
                .rename_axis(index=self.index),
                "found": self.particles.loc()[("gridded", slice(None)), self.position_names].drop_duplicates(),
            },
            axis=1,
        )
        pd.testing.assert_frame_equal(offsets["expected"], offsets["found"], check_dtype=False)
